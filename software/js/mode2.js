...
* Functions: 		select_waypoints(), track_drone(), send_waypoints(),
                    toggle_options(), detect_pose(), main()
...
const detection = require(path.join(__dirname, "../js", "detection"));
...
// defining buttons
const detect_pose = document.getElementById('detectPoseBtn');
...
* Input: 		None
* Output: 		Alerts the users that pose detection is ON
* Logic: 		This function is called when the detect_pose button is clicked.
                It calls the loadPoseNet method and detectPose method
* Example Call:		detect_pose(checkbox)
*
*/
detect_pose.addEventListener('click', async () => {
    const net = await detection.load_posenet();
    setInterval(() => {
        detection.detect_pose(net);
    }, 1000);
...
send_waypoints.addEventListener('click', () => {
    telemetry.send_waypoints(waypoints, 8, 2);
});
